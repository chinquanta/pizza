...
def test_init():
    ...
    assert pizza_exists.recipe == None
    assert pizza_exists.pict == '🍕'
    assert Pizza._recipes.get('Pepperoni') == None
    assert Pizza._picts.get('Pepperoni') == None
    pizza_exists.recipe = {'mozzarella', 'pepperoni', 'tomato sauce'}
    assert pizza_exists.recipe == {'mozzarella', 'pepperoni', 'tomato sauce'}
    assert Pizza._recipes.get('Pepperoni') == {'mozzarella', 'pepperoni', 'tomato sauce'}
    assert Pizza._picts.get('Pepperoni') == '🍕'

@pytest.mark.parametrize('s,exp',
                         [(('XXL', 'Margherita', '🧀'), pytest.raises(ValueError)),
                          ((100, 'Margherita', '🧀'), pytest.raises(ValueError))])
